...
public class ItemPickUp : MonoBehaviour
{
    ...
    [SerializeField] KeyCode TeleportKey = KeyCode.Space;
    ...
    private void Update()
    {
        if(Input.GetKeyDown(pickUpKey))
        {
            ...
            {
                RaycastHit hit;
                if ((Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, pickUpRange)) && hit.collider.gameObject.tag == "PickUpAble")
                {
